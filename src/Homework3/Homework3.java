...
public class Homework3
{
    ...
    private static void arrOffset(int n, int... arrOffset)
    {
        if (n > 0)
        {
            for (int i = arrOffset.length - 1; i >= arrOffset.length - n; i--)
            {
                int t = arrOffset[i - (arrOffset.length - n)];
                arrOffset[i - (arrOffset.length - n)] = arrOffset[i];
                arrOffset[i] = t;
            }
            for (int i = n; i < arrOffset.length - n; i++)
            {
                int t = arrOffset[i + n];
                arrOffset[i + n] = arrOffset[i];
                arrOffset[i] = t;
            }
        }
        if (n < 0)
